...
const EditorSection = (props) => {
    ...
    const handleChange = (e) => {
        setState({ ...state, editorState: e.target.value })

    }

    const handleSubmit = (e) => {
        alert(state.editorState);
    }
        ...
        <div className="flex justify-center max-h-[calc(100vh-112px)] w-[65%] overflow-y-auto" >
            <div className="demo-inline">
                <div className="container">
                    <ContentEditable className="editable tinymce-body" onChange={(e) => handleChange(e)} html={state.editorState} spellCheck />
                </div>
            </div>
            {/* <button onClick={fontSizeChange}>Font Size</button> */}
        </div>
...
const Optional = (props) => {
    ...
    const fontChange = function () {
        tinymce.activeEditor.execCommand("FontSize", false, `${state.fSize}px`);
    }
